...
<modal v-if="deleteModal" @close="clearAll()">
    <h3 slot="head">APPROVE</h3>
    <div slot="body" class="text-center">
        <div class="form-group">
            <input type="hidden" class="form-control" name="status" v-model="chooseItem.status">
            <input type="hidden" class="form-control" name="request_id" v-model="chooseItem.request_id">
             <input type="hidden" class="form-control" name="item_id" v-model="chooseItem.item_id">
               <input type="hidden" class="form-control" name="quantity" v-model="chooseItem.quantity">

            </div>

      Do you want to APPROVE this record?</div>
    <div slot="foot">
        <button class="btn btn-dark" @click="deleteModal = false; deleteUser()" >APPROVE</button>
